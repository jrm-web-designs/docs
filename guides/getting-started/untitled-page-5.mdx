...
|-------|------|-------------|
| `rate_limit_per_minute` | `number` | Maximum messages you can send per minute. Exceeding this limit returns a `RATE_LIMIT_EXCEEDED` error. |
| `rate_limit_per_hour` | `number` | Maximum messages you can send per hour. Applies after ramp-up period. |
| `daily_limit` | `number` | Maximum messages you can send per day. May be configured for your workspace. |
| `ramp_limit_per_hour` | `number` | Initial hourly limit for new workspaces. Applied during ramp-up period. |
...
- [Concepts](/docs/getting-started/concepts) — Understand core concepts
- [Troubleshooting](/docs/troubleshooting/common-issues) — Common issues and solutions
- [API Reference: Workspace Status](/docs/api-reference/workspace-status) — Status API documentation
